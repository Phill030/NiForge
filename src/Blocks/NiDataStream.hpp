...
public:
    ...
    SemanticData(Reader& reader, const NiHeader& header) {
...
public:
    ...
    Region() = default;
    Region(Reader& reader, const NiHeader& header) {
...
public:
	...
	std::vector<uint16_t> submeshToRegionMap;
    uint32_t numComponents;
    std::vector<SemanticData> componentSemantics;

    DataStreamRef(Reader& reader, const NiHeader& header) : stream(Ref<NiDataStream>(reader)) {
